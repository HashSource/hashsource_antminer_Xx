...
GREEN_LED=gpio23
BEEP=gpio50
# RED LED: GPIO1_13: $RED_LED
if [ ! -d /sys/class/gpio/$RED_LED ]; then
    echo 45 > /sys/class/gpio/export
    echo out > /sys/class/gpio/$RED_LED/direction
fi

# GREEN LED: GPIO0_23: $GREEN_LED
if [ ! -d /sys/class/gpio/$GREEN_LED ]; then
    echo 23 > /sys/class/gpio/export
    echo out > /sys/class/gpio/$GREEN_LED/direction 
fi

if [ ! -d /sys/class/gpio/$BEEP ]; then
    echo 45 > /sys/class/gpio/export
    echo out > /sys/class/gpio/$BEEP/direction
fi

echo 0 > /sys/class/gpio/$BEEP/value
echo 1 > /sys/class/gpio/$RED_LED/value
echo 1 > /sys/class/gpio/$GREEN_LED/value
...
do
	echo 0 > /sys/class/gpio/$BEEP/value
	echo 0 > /sys/class/gpio/$GREEN_LED/value
	echo 0 > /sys/class/gpio/$RED_LED/value
	sleep 1s
	echo 1 > /sys/class/gpio/$BEEP/value
	echo 1 > /sys/class/gpio/$GREEN_LED/value
	echo 1 > /sys/class/gpio/$RED_LED/value
	sleep 1s
done
